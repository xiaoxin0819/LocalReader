/* publish-release.mjs —— 把 dist/LocalReader.zip 发到 GitHub Release
   ...
   可选环境变量：
     GITHUB_REPO   默认 xiaoxin0819/LocalReader
...
const REPO = process.env.GITHUB_REPO || 'xiaoxin0819/LocalReader';
...
const headers = {
  ...
  'User-Agent': 'LocalReader-Publisher',
...
try {
  ...
  const body = notesPath && fs.existsSync(notesPath)
    ...
    : [
      `## LocalReader ${TAG}`,
      '',
      '下载下面的 **LocalReader.zip**，解压后双击 `LocalReader.exe` 即可（无需安装 Node.js）。',
      '',
      '- 首次运行自动释放内置的 31 个书源',
...
/* 4) 上传新附件 */
step('上传 LocalReader.zip…');
const uploadUrl = `https://uploads.github.com/repos/${REPO}/releases/${release.id}/assets?name=LocalReader.zip`;
...
console.log('  最新: https://github.com/' + REPO + '/releases/latest/download/LocalReader.zip');
