...
function renderShelfSelect() {
  ...
  if (!state.shelves.length) {
    ...
    $("shelfPath").textContent = "点右上「导入文件夹」添加";
    return;
  }
...
function prefetchNeighbors(idx) {
  ...
  const jobs = [];
  if (idx + 1 < total) jobs.push(idx + 1);
  if (idx - 1 >= 0) jobs.push(idx - 1);
  if (idx === 0) jobs.push(idx + 1);
  const run = () => jobs.forEach((i) => { if (!chapterCache.has(chapterKey(b.rel, i))) fetchChapter(b.rel, i); });
...
async function runChapter(idx, scrollTo = 0) {
  ...
  /* 只有「滚轮滚到章界」才做连续滚动：旧章与新章拼在同一层里，
     ...
     毫无区别，不插标记的话翻章根本看不出来。
     点按 / 键盘 / 目录跳转等直接换章（瞬时）。 */
  const atBoundary = dir > 0 ? scrollTo === 0 : scrollTo > 0;
  let animated = false;
...
function gotoChapter(idx, scrollTo = 0) {
  const now = performance.now();
  if (gateTimer === null && now - gateLast >= GATE_MS) {
    gateLast = now;
    return runChapter(idx, scrollTo);
  }
  gateArgs = [idx, scrollTo];
  return new Promise((resolve) => {
    ...
    if (gateTimer === null) {
      ...
      gateTimer = setTimeout(async () => {
        ...
        const args = gateArgs || [state.chapterIdx, 0];
        gateArgs = null;
        const ws = gateWaiters; gateWaiters = [];
        try { await runChapter(args[0], args[1]); } catch {}
        ws.forEach((w) => w());
      }, wait);
    }
  });
...
$("content").addEventListener("wheel", (e) => {
  ...
  if (e.deltaY > 0) {
    ...
    return gotoChapter(state.chapterIdx + 1);
  }
  ...
  gotoChapter(state.chapterIdx - 1, 1);
}, { passive: false });
...
$("content").addEventListener("contextmenu", (e) => {
  ...
  showCtx(e.clientX, e.clientY, [
    ...
    { label: state.book.name + (state.book.author ? "  ·  " + state.book.author : ""), info: true, disabled: true, act: () => {} },
    {
